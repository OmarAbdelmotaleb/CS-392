...
static void swap(void *a, void *b, size_t size) { //am i very skeptical this is right
	//puts("Starting swap"); //Getting a segfault somewhere in here, as to be expected
 ...
 * pointer arithmetic. *(array + i) < pivot 
 * Use ((char*)array)[i] or &(((char*)array)[i]) -> ((char*)array + i*elem_sz)
 */
static int lomuto(void *array, int left, int right, size_t elem_sz,
                  int (*comp) (const void*, const void*)) {
    char* pivot =  (char*)array + left * elem_sz; //tells us what address pivot is at
    int s = left;
...
static void quicksort_helper(void *array, int left, int right, size_t elem_sz,
                             int (*comp) (const void*, const void*)) {
    // TODO
    //puts("Starting quicksort_helper");
    if (left < right) {
...
void quicksort(void *array, size_t len, size_t elem_sz,
    ...
    //do we need to do &array or is it fine
}
